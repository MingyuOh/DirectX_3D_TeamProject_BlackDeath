...
struct BGeomMesh
{
	...
	vector<TVector3>	m_pNorVertexList;
...
public:
	...
	/*오브젝트 로드에 사용되는 함수*/
	bool GetVertexListFromString(TCHAR* strVertexList,DWORD dwNumFace,vector<TVector3>& VertexList,TCHAR* strToken = _T("*"));
	...
	bool SetCamera(TMatrix Camera_matView, TMatrix Camera_matProj);
	...
	void Interpolate(BMesh2* pMesh, TMatrix* matParent, float fFrameTick);
	...
	void SettingCameraForNormal(TVector3 Pos, TVector3 Look);
