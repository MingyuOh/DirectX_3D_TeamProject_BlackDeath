...
#define MAX_BONE_MATRIX_COUNT 50
...
VS_OUTPUT VS(VS_INPUT vln)
{
	...
	if ((uint)vln.w2.w != 0)
	{
		for (int iBipedCnt = 0; iBipedCnt < (uint)vln.w2.w; iBipedCnt++)
		{
			...
			if (iBipedCnt < 4)
			{
				matMatrix = m_matConstBoneWorld[iBoneIndex];// FetchboneTransform(iBoneIndex);
	...
	//Output.Position = mul(Output.Position, g_matInverse);	//최종월드
	//NorResult = mul(NorResult, g_matInverse);	//최종월드
	//TanResult = mul(TanResult, g_matInverse);	//최종월드
...
VS_OUTPUT VS_NORMAL(VS_INPUT vln)
{
	...
	/*===================== 접선공간 변환(노말맵) ======================*/
	float3 vNormal = normalize(mul(NorResult, (float3x3)g_matWorldInverse));
	float3 T = normalize(mul(TanResult, (float3x3)g_matWorldInverse));
