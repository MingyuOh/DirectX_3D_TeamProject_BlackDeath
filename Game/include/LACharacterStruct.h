...
struct PNCT5_VERTEX
{
	...
	PNCT5_VERTEX()
	{
		...
		ZeroMemory(&vTangent, sizeof(D3DXVECTOR4));
	}
	PNCT5_VERTEX(D3DXVECTOR4 dxVertex,
		D3DXVECTOR4 dxNomal,
		D3DXVECTOR4 dxColor,
		D3DXVECTOR2 dxTexture,
		D3DXVECTOR4 dxTangent)
	{
		Vertex = dxVertex;
		Nomal = dxNomal;
		Color = dxColor;
...
struct CBConstBoneWorld
{
	...
	CBConstBoneWorld()
	{
		ZeroMemory(&g_mConstBoneWorld, sizeof(D3DXMATRIX) * MAX_BONE_MATRICES);
	}
};
...
struct ObjectRenderData
{
	...
	ObjectRenderData()
	{
		iCurrentScene = 0;
		iCurrentTick = 0;
		ZeroMemory(&matObjectWorld, sizeof(D3DXMATRIX));
	}
};
...
typedef struct _D3D_MATRIX {
	...
	void operator = (D3DXMATRIX& rmatrix)
	{
...
struct StructV2
{
	...
	void operator = (D3DXVECTOR2& v2)
	{
		x = (int)(v2.x * 1000.0f);
		y = (int)(v2.y * 1000.0f);
	}

	D3DXVECTOR2 D3DXVEC2()
	{
		D3DXVECTOR2 data;
...
{
	int InstancingCount;
	D3DXMATRIX matNormalWorldInverse[g_iMaxRenderMonsterCount];	//노말 접선공간 변환에 사용
	D3DXMATRIX matBoneAnimationWorld[g_iMaxRenderMonsterCount * g_iMaxBoneMatrixCount];	//애니메이션

	InstancingDataInfo()
	{
		ZeroMemory(&matNormalWorldInverse, sizeof(D3DXMATRIX) * g_iMaxRenderMonsterCount);
		ZeroMemory(&matBoneAnimationWorld, sizeof(D3DXMATRIX) * g_iMaxRenderMonsterCount * g_iMaxBoneMatrixCount);
		InstancingCount = 0; 
	}
};