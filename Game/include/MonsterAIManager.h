...
public:
	...
	D3DXVECTOR3 SetMonsterLook(D3DXVECTOR3 HeroPos, D3DXVECTOR3 MonPos);
	float DistancePointtoPoint(D3DXVECTOR3 Point_1, D3DXVECTOR3 Point_2);
	void bubble_sort(float arr[], D3DXVECTOR3 arr2[], int count);		//거품정렬
	int ReturnMostCloseHerotoSpawnPoint(D3DXVECTOR3 HeroPos);	//히어로의 위치와 가장 가까운 스폰지점을 리턴
	int ReturnRandomSpawn(D3DXVECTOR3 MonPos);	//현재 위치를 제외한 0~6중 랜덤한 숫자를 리턴
	void MakeGraph();
	//D3DXVECTOR3 ReturnMostCloseSpawnPoint(D3DXVECTOR3 CurrentSpawnPoint);

	D3DXVECTOR3 MonsterPathing(int Path);
